...
def _estimate_surface_normals(points):
    from scipy.spatial import cKDTree
    tree=cKDTree(points); neighbours=tree.query(points,k=min(24,len(points)),workers=-1)[1]; normals=np.empty_like(points)
    for index,neighbour_ids in enumerate(neighbours):
        local=points[neighbour_ids]; centered=local-local.mean(axis=0); _,vectors=np.linalg.eigh(centered.T@centered/max(1,len(local)-1)); normals[index]=vectors[:,0]
...
def _fuse_dense_surface(field_uv,field_camera,field_depth,field_world,masks,cameras,stride,config):
    import cv2
    uv=np.asarray(field_uv,np.float32); source=np.asarray(field_camera,np.int16); depth=np.asarray(field_depth,np.float32); points=np.asarray(field_world,np.float32)
    camera_count=len(cameras); grids=[]
    for camera_index,mask in enumerate(masks):
        grid=np.full(((mask.shape[0]-1)//stride+1,(mask.shape[1]-1)//stride+1),np.nan,np.float32); selected=source==camera_index; pix=np.rint(uv[selected]).astype(np.int64); grid[pix[:,1]//stride,pix[:,0]//stride]=depth[selected]; grids.append(grid)
    low=np.percentile(points,1,axis=0); high=np.percentile(points,99,axis=0); object_scale=float(np.linalg.norm(high-low)); spacing=float(config.get("fusion_relative_sample_spacing",0.006)*object_scale); depth_range=float(np.percentile(depth,99)-np.percentile(depth,1)); tolerance=max(spacing*float(config.get("fusion_depth_tolerance_factor",1.0)),depth_range*1e-3)
    visible=np.zeros((len(points),camera_count),bool); errors=np.full((len(points),camera_count),np.nan,np.float32); projected=np.full((len(points),camera_count,2),np.nan,np.float32)
    for camera_index,camera in enumerate(cameras):
        ...
        target_uv,_=cv2.projectPoints(points.reshape(-1,1,3),cv2.Rodrigues(rotation)[0],translation,intrinsic,distortion); target_uv=target_uv.reshape(-1,2); target_depth=(rotation@points.T+translation)[2]; projected[:,camera_index]=target_uv
        ...
        reference=_sample_stride_depth(grids[camera_index],target_uv,stride); error=np.abs(target_depth-reference); errors[:,camera_index]=error; visible[:,camera_index]=in_roi&np.isfinite(error)&(error<=tolerance)
    ...
    if len(selected)>maximum: selected=selected[np.argsort(mean_error[first])[:maximum]]
    fused=points[selected]; normals=_estimate_surface_normals(fused); return {"points":fused,"normals":normals,"source_camera":source[selected],"visibility_mask":visible[selected],"projected_uv":projected[selected],"depth_abs_error":errors[selected],"visible_counts":counts[selected].astype(np.int16),"object_scale":object_scale,"sample_spacing":spacing,"depth_tolerance":tolerance,"candidate_count":int(len(candidate))}
...
def pretrain_ndef_surface(config="config/ndef_multiview.yaml"):
    ...
    masks=[]; queries=[]; query_c=[]; stride=int(values.get("surface",{}).get("pretrain_query_stride",4))
    ...
    positive_roi=roi_support&(depth>0)
    ...
    med=np.median(depth[positive_roi]); mad=np.median(np.abs(depth[positive_roi]-med)); depth_clean=(depth>0)&(np.abs(depth-med)<=6*max(mad,1e-6)); keep=roi_support&depth_clean
    ...
    if int(dense.get("iterations", 0)) > 0:
        import cv2
        ...
        mapping={"iterations":"dense_iterations","samples_per_camera":"dense_samples_per_camera","spacing_px":"dense_spacing_px","patch_radius":"dense_patch_radius","learning_rate":"dense_learning_rate","anchor_weight":"dense_anchor_weight","min_valid_patch_ratio":"dense_min_valid_patch_ratio","seed":"dense_seed"}
    ...
    (pretrain_out/"surface_pretrain_meta.json").write_text(json.dumps({"kept_sparse_observations":int(keep.sum()),"rejected_outside_roi":int((~roi_support).sum()),"rejected_depth_outliers":int((roi_support&(~depth_clean)).sum()),"depth_median":float(med),"depth_mad":float(mad),"depth_normalization":{"mean":float(r.depth_mean),"std":float(r.depth_std)},"roi_uv_normalization_bounds":roi_uv_bounds.tolist(),"calibration_reprojection_px":reprojection,"diagnostics":{"sparse_rmse":float(r.diagnostics.metrics["sparse_rmse"])}},indent=2))
    if r.dense_uv is not None and r.dense_uv.numel() > 0:
        ...
        (out/"surface_dense_meta.json").write_text(json.dumps({"config":dense,"normalization":{"roi_uv_bounds":roi_uv_bounds.tolist(),"depth_mean":float(r.depth_mean),"depth_std":float(r.depth_std)},"calibration_reprojection_px":reprojection,"diagnostics":dict(r.diagnostics.metrics),"history_steps":int(r.dense_history.shape[0]),"history_columns":["photo_loss","anchor_loss","total_loss"],"fusion":{"relative_sample_spacing":float(values.get("surface",{}).get("fusion_relative_sample_spacing",.006)),"sample_spacing":float(fusion["sample_spacing"]),"depth_tolerance":float(fusion["depth_tolerance"]),"min_visible_cameras":int(values.get("surface",{}).get("fusion_min_visible_cameras",2)),"candidate_count":int(fusion["candidate_count"]),"surface_point_count":int(len(fusion["points"]))}},indent=2))
