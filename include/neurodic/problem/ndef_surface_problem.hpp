...
public:
    ...
    int pretrain_iterations{10000}; double pretrain_learning_rate{1e-3}, weight_decay{1e-6}, smoothness_weight{0.0};
    ...
    int dense_epochs{100};
    // Manual per-camera batch size. In auto-batch mode the full largest shard
    // is the probe ceiling, matching the Python NDeF-DIC scheduler.
    int dense_samples_per_camera{10000};
    bool dense_auto_batch{true};
    int dense_auto_batch_start{1};
    double dense_memory_fraction{0.8};
